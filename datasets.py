...
# Replace your old get_dataset function with this one

def get_dataset(name, root, train, flip=False, crop=False, download=False):
    if name == 'cifar':
        DATASET = CIFAR10
        tf = []
        if train:
            tf.append(transforms.Resize(32))
            if crop:
                tf.append(transforms.RandomCrop(32, padding=4, padding_mode='reflect'))
            if flip:
                tf.append(transforms.RandomHorizontalFlip())
        else:
            tf.append(transforms.Resize(32))
        tf.append(transforms.ToTensor())
        tf.append(transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)))
    else:
        raise NotImplementedError(name)
    
    # The 'download=download' part is the fix
